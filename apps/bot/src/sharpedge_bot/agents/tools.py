...
from sharpedge_analytics import (
    analyze_key_numbers as analyze_key_numbers_fn,
)
from sharpedge_analytics import (
...
from sharpedge_db.models import BetHistoryParams
from sharpedge_db.queries.arbitrage import get_active_arbitrage as get_active_arbitrage_db
...
from sharpedge_db.queries.line_movements import (
    get_line_movements as get_line_movements_db,
)
from sharpedge_db.queries.line_movements import (
    get_movement_summary,
    get_recent_steam_moves,
)
from sharpedge_db.queries.opening_lines import (
    calculate_movement_from_open,
)
from sharpedge_db.queries.opening_lines import (
    get_opening_line as db_get_opening_line,
)
from sharpedge_db.queries.public_betting import (
    get_public_betting as db_get_public_betting,
)
from sharpedge_db.queries.public_betting import (
    get_sharp_plays as db_get_sharp_plays,
)
from sharpedge_db.queries.value_plays import get_active_value_plays as db_get_active_value_plays
...
def get_current_odds(sport: str, game_query: str) -> str:
    ...
    try:
        ...
        sport_filter = SportEnum(sport) if sport else None
        game = client.find_game(game_query, sport_filter)
        if not game:
        ...
        return json.dumps(
            {
                ...
                "spreads": [
                    {
                        "book": quote.sportsbook_display,
                        "line": quote.line,
                        "odds": quote.odds,
                        "side": quote.side,
                    }
                    for quote in (comparison.spread_home + comparison.spread_away)
                ],
                "totals": [
                    {
                        "book": quote.sportsbook_display,
                    ...
                    for quote in (comparison.total_over + comparison.total_under)
                ],
                "moneylines": [
                    {
                        "book": quote.sportsbook_display,
                    ...
                    for quote in (comparison.moneyline_home + comparison.moneyline_away)
                ],
            },
            default=str,
        )
...
def query_recent_bets(user_id: str, limit: int = 20) -> str:
    ...
    bets = get_bet_history(
        BetHistoryParams.for_query(user_id=user_id, limit=limit),
    )
    return json.dumps(
        [
            {
...
def get_opening_line(game_id: str, bet_type: str = "spread") -> str:
    ...
    """
    opening = db_get_opening_line(game_id, bet_type)
    if not opening:
...
def get_public_betting(game_id: str) -> str:
    ...
    Args:
        game_id: Game identifier
    """
    data = db_get_public_betting(game_id)
    if not data:
...
def get_sharp_plays(min_divergence: float = 10, sport: str | None = None) -> str:
    ...
    """
    plays = db_get_sharp_plays(min_divergence=min_divergence, sport=sport)
...
def get_line_movements(game_id: str, significant_only: bool = True) -> str:
    ...
    """
    movements = get_line_movements_db(game_id, significant_only=significant_only)
...
def get_active_value_plays(sport: str | None = None, min_ev: float = 2.0) -> str:
    ...
    """
    plays = db_get_active_value_plays(sport=sport, min_ev=min_ev)
    return json.dumps(
        [
            {
                "game": p.get("game"),
...
def get_active_arbitrage(min_profit: float = 0.5) -> str:
    ...
    """
    arbs = get_active_arbitrage_db(min_profit=min_profit)
    return json.dumps(
        [
            {
                "game": a.get("game"),
...
@function_tool
def check_arbitrage(
    odds_a: int, odds_b: int, book_a: str = "Book A", book_b: str = "Book B"
) -> str:
...
def analyze_key_numbers(line: float, sport: str = "NFL") -> str:
    ...
    Args:
        ...
        sport: Sport code
    """
    analysis = analyze_key_numbers_fn(line, sport)
    return json.dumps(
        {
...
def get_weather_impact(game_id: str) -> str:
    ...
    result = client.table("game_weather").select("*").eq("game_id", game_id).limit(1).execute()
